...
def model_results(F, region, data, calib, res_name, runs):
    ...
    ## Run the model
    P = at.Project(framework=F, databook=f"applications/{region}/" + data, sim_start=1990, sim_end=2099, sim_dt=0.25,
                   do_run=False)
    cal = P.make_parset()
    ...
    out_res = [{"pop": ["alive", "total", "sum"]},  # total population size
               ...
               # disease management, no treatment (for costs)
               {"dm_tx": [[{"dm_tx": "icl_tx+ict_tx+ie_tx+cc_tx+dc_tx+hcc_tx"}], "total", "sum"]},
               # disease management, treatment (for costs)
               {"pop_hcc": [[{"pop_hcc": "cc_dx+cc_tx+dc_dx+dc_tx"}], "total", "sum"]},
               # population HCC surveillance (for costs)
               {"tgt_hcc": [[{"tgt_hcc": "it_dx+icl_dx+icl_tx+ict_dx+ict_tx+ie_dx+ie_tx"}],
                            {"50+": ["50-69M", "50-69F", "70+M", "70+F"]}, "sum"]},
               # targeted HCC surveillance (for costs)
               {"tgt_hcc_b": [[{"tgt_hcc": "it_dx+icl_dx+icl_tx+ict_dx+ict_tx+ie_dx+ie_tx"}],
                              {"40+": ["15-49M", "15-49F"]}, "sum"]},  # targeted HCC surveillance (for costs (40-49))
               ...
               # hospitalization among untreated (for costs)
               ...
               # number of births recieving mAVs and HBIG,
...
def econ_analysis(cent, cent_s1, cent_s2, cent_s3, res, res_s1, res_s2, res_s3, reg, wd, cost_data, runs, cdisc_rate,
                  hdisc_rate):
    # Discounting Array
    if cdisc_rate > 1:
    ...
    cbd_cost_bl, chb3_cost_bl, cbd_cost_s1, chb3_cost_s1, cbd_cost_s2, chb3_cost_s2, cbd_cost_s3, chb3_cost_s3 = np.zeros(
        (109, 1)), np.zeros((109, 1)), np.zeros((109, 1)), np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1)), np.zeros((109, 1))
    bd_cost_bl, hb3_cost_bl, bd_cost_s1, hb3_cost_s1, bd_cost_s2, hb3_cost_s2, bd_cost_s3, hb3_cost_s3 = np.zeros(
        (109, runs)), np.zeros((109, runs)), np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs)), np.zeros((109, runs)), np.zeros((109, runs))
    ...
    cmsc_cost_bl, cmsc_cost_s1, cmsc_cost_s2, cmsc_cost_s3 = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    msc_cost_bl, msc_cost_s1, msc_cost_s2, msc_cost_s3 = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))

    cmsc_cost_bl[:, 0] = (cent["prg_scr"][:] * dx_cost * discount[:,
                                                         1])  # (cent["mav_n"][:]*mav_cost*discount[:,1])+(cent["prg_hrs"][:]*hrs_cost*discount[:,1])
    cmsc_cost_s1[:, 0] = (cent_s1["mav_n"][:] * mav_cost * discount[:, 1]) + (
                cent_s1["prg_scr"][:] * dx_cost * discount[:, 1]) + (cent_s1["prg_hrs"][:] * hrs_cost * discount[:, 2])
    cmsc_cost_s2[:, 0] = (cent_s2["mav_n"][:] * mav_cost * discount[:, 1]) + (
                cent_s2["prg_scr"][:] * dx_cost * discount[:, 1]) + (cent_s2["prg_hrs"][:] * hrs_cost * discount[:, 2])
    cmsc_cost_s3[:, 0] = (cent_s3["mav_n"][:] * mav_cost * discount[:, 1]) + (
                cent_s3["prg_scr"][:] * dx_cost * discount[:, 1]) + (cent_s3["prg_hrs"][:] * hrs_cost * discount[:, 2])

    for run in range(runs):
        msc_cost_bl[:, run] = (res["prg_scr"][:, run] * dx_cost * discount[:,
                                                                  1])  # (res["mav_n"][:,run]*mav_cost*discount[:,1])+(res["prg_hrs"][:,run]*hrs_cost*discount[:,1])
        msc_cost_s1[:, run] = (res_s1["mav_n"][:, run] * mav_cost * discount[:, 1]) + (
                    res_s1["prg_scr"][:, run] * dx_cost * discount[:, 1]) + (
                                          res_s1["prg_hrs"][:, run] * hrs_cost * discount[:, 2])
        msc_cost_s2[:, run] = (res_s2["mav_n"][:, run] * mav_cost * discount[:, 1]) + (
                    res_s2["prg_scr"][:, run] * dx_cost * discount[:, 1]) + (
                                          res_s2["prg_hrs"][:, run] * hrs_cost * discount[:, 2])
        msc_cost_s3[:, run] = (res_s3["mav_n"][:, run] * mav_cost * discount[:, 1]) + (
                    res_s3["prg_scr"][:, run] * dx_cost * discount[:, 1]) + (
                                          res_s3["prg_hrs"][:, run] * hrs_cost * discount[:, 2])

    # Diagnosis Costs (note three approaches coded, final one [dx_costb] used for analysis [proportional negative testing costs])
    cdx_cost_bl, cdx_cost_s1, cdx_cost_s2, cdx_cost_s3 = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    dx_cost_bl, dx_cost_s1, dx_cost_s2, dx_cost_s3 = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))
    ...
    cdx_costa_bl, cdx_costa_s1, cdx_costa_s2, cdx_costa_s3 = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    dx_costa_bl, dx_costa_s1, dx_costa_s2, dx_costa_s3 = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))
    ...
    cbl_dx_inc, cs1_dx_inc, cs2_dx_inc, cs3_dx_inc = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    bl_dx_inc, s1_dx_inc, s2_dx_inc, s3_dx_inc = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))
    ...
    cdx_costb_bl, cdx_costb_s1, cdx_costb_s2, cdx_costb_s3 = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    dx_costb_bl, dx_costb_s1, dx_costb_s2, dx_costb_s3 = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))

    for yr in range(len(cdx_costb_bl)):
        cdx_costb_bl[yr, 0] = (cent["dx_rate"][yr] * dx_cost * discount[yr, 1]) + (
                    dx_cost * cbl_dx_inc[yr, 0] * (cent["pop"][yr] - cent["dx_rate"][yr]) * discount[yr, 1])
        cdx_costb_s1[yr, 0] = (cent_s1["dx_rate"][yr] * dx_cost * discount[yr, 1]) + (
                    dx_cost * cs1_dx_inc[yr, 0] * (cent_s1["pop"][yr] - cent_s1["dx_rate"][yr]) * discount[yr, 1])
        cdx_costb_s2[yr, 0] = (cent_s2["dx_rate"][yr] * dx_cost * discount[yr, 1]) + (
                    dx_cost * cs2_dx_inc[yr, 0] * (cent_s2["pop"][yr] - cent_s2["dx_rate"][yr]) * discount[yr, 1])
        cdx_costb_s3[yr, 0] = (cent_s3["dx_rate"][yr] * dx_cost * discount[yr, 1]) + (
                    dx_cost * cs3_dx_inc[yr, 0] * (cent_s3["pop"][yr] - cent_s3["dx_rate"][yr]) * discount[yr, 1])

    for run in range(runs):
        for yr in range(len(dx_costb_bl)):
            dx_costb_bl[yr, run] = (res["dx_rate"][yr, run] * dx_cost * discount[yr, 1]) + (
                        dx_cost * bl_dx_inc[yr, run] * (res["pop"][yr, run] - res["dx_rate"][yr, run]) * discount[
                    yr, 1])
            dx_costb_s1[yr, run] = (res_s1["dx_rate"][yr, run] * dx_cost * discount[yr, 1]) + (
                        dx_cost * s1_dx_inc[yr, run] * (res_s1["pop"][yr, run] - res_s1["dx_rate"][yr, run]) * discount[
                    yr, 1])
            dx_costb_s2[yr, run] = (res_s2["dx_rate"][yr, run] * dx_cost * discount[yr, 1]) + (
                    dx_cost * s2_dx_inc[yr, run] * (res_s2["pop"][yr, run] - res_s2["dx_rate"][yr, run]) * discount[
                yr, 1])
            dx_costb_s3[yr, run] = (res_s3["dx_rate"][yr, run] * dx_cost * discount[yr, 1]) + (
                    dx_cost * s3_dx_inc[yr, run] * (res_s3["pop"][yr, run] - res_s3["dx_rate"][yr, run]) * discount[
                yr, 1])

    # Treatment Costs
    tx_cost = care_costs[reg].iloc[3]

    ctx_cost_bl, ctx_cost_s1, ctx_cost_s2, ctx_cost_s3 = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    tx_cost_bl, tx_cost_s1, tx_cost_s2, tx_cost_s3 = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))
    ...
    cmc_cost_bl, cmc_cost_s1, cmc_cost_s2, cmc_cost_s3 = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    chosp_cost_bl, chosp_cost_s1, chosp_cost_s2, chosp_cost_s3 = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    chcc_cost_bl, chcc_cost_s1, chcc_cost_s2, chcc_cost_s3 = np.zeros((109, 1)), np.zeros((109, 1)), np.zeros(
        (109, 1)), np.zeros((109, 1))
    ...
    chosp_cost_bl[:, 0] = ((cent["hsp_utx"][:] * hosp_cost * util) + (
                cent["hsp_tx"][:] * hosp_cost * util * tx_hosp)) * discount[:, 1]
    chosp_cost_s1[:, 0] = ((cent_s1["hsp_utx"][:] * hosp_cost * util) + (
                cent_s1["hsp_tx"][:] * hosp_cost * util * tx_hosp)) * discount[:, 1]
    chosp_cost_s2[:, 0] = ((cent_s2["hsp_utx"][:] * hosp_cost * util) + (
                cent_s2["hsp_tx"][:] * hosp_cost * util * tx_hosp)) * discount[:, 1]
    chosp_cost_s3[:, 0] = ((cent_s3["hsp_utx"][:] * hosp_cost * util) + (
                cent_s3["hsp_tx"][:] * hosp_cost * util * tx_hosp)) * discount[:, 1]
    ## HCC Surveillance Costs (assume 50% HR covered under UHC; 25% resource utilization)
    chcc_cost_bl[:, 0] = ((cent["pop_hcc"][:] * hcc_cost * util) + (cent["tgt_hcc"][:] * hcc_cost * util) + (
                cent["tgt_hcc_b"][:] * hcc_prp * hcc_cost * util)) * discount[:, 1]
    chcc_cost_s1[:, 0] = ((cent_s1["pop_hcc"][:] * hcc_cost * util) + (cent_s1["tgt_hcc"][:] * hcc_cost * util) + (
                cent_s1["tgt_hcc_b"][:] * hcc_prp * hcc_cost * util)) * discount[:, 1]
    chcc_cost_s2[:, 0] = ((cent_s2["pop_hcc"][:] * hcc_cost * util) + (cent_s2["tgt_hcc"][:] * hcc_cost * util) + (
                cent_s2["tgt_hcc_b"][:] * hcc_prp * hcc_cost * util)) * discount[:, 1]
    chcc_cost_s3[:, 0] = ((cent_s3["pop_hcc"][:] * hcc_cost * util) + (cent_s3["tgt_hcc"][:] * hcc_cost * util) + (
                cent_s3["tgt_hcc_b"][:] * hcc_prp * hcc_cost * util)) * discount[:, 1]

    mc_cost_bl, mc_cost_s1, mc_cost_s2, mc_cost_s3 = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))
    hosp_cost_bl, hosp_cost_s1, hosp_cost_s2, hosp_cost_s3 = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))
    hcc_cost_bl, hcc_cost_s1, hcc_cost_s2, hcc_cost_s3 = np.zeros((109, runs)), np.zeros((109, runs)), np.zeros(
        (109, runs)), np.zeros((109, runs))

    for run in range(runs):
        ## Disease Management Costs
        mc_cost_bl[:, run] = ((res["dm_dx"][:, run] * dx_dmc * util) + (
                    res["dm_tx"][:, run] * tx_dmc * util)) * discount[:, 1]
        mc_cost_s1[:, run] = ((res_s1["dm_dx"][:, run] * dx_dmc * util) + (
                    res_s1["dm_tx"][:, run] * tx_dmc * util)) * discount[:, 1]
        mc_cost_s2[:, run] = ((res_s2["dm_dx"][:, run] * dx_dmc * util) + (
                    res_s2["dm_tx"][:, run] * tx_dmc * util)) * discount[:, 1]
        mc_cost_s3[:, run] = ((res_s3["dm_dx"][:, run] * dx_dmc * util) + (
                    res_s3["dm_tx"][:, run] * tx_dmc * util)) * discount[:, 1]
        ## Hospitalisation Costs
        hosp_cost_bl[:, run] = ((res["hsp_utx"][:, run] * hosp_cost * util) + (
                    res["hsp_tx"][:, run] * hosp_cost * util * tx_hosp)) * discount[:, 1]
        hosp_cost_s1[:, run] = ((res_s1["hsp_utx"][:, run] * hosp_cost * util) + (
                    res_s1["hsp_tx"][:, run] * hosp_cost * util * tx_hosp)) * discount[:, 1]
        hosp_cost_s2[:, run] = ((res_s2["hsp_utx"][:, run] * hosp_cost * util) + (
                    res_s2["hsp_tx"][:, run] * hosp_cost * util * tx_hosp)) * discount[:, 1]
        hosp_cost_s3[:, run] = ((res_s3["hsp_utx"][:, run] * hosp_cost * util) + (
                    res_s3["hsp_tx"][:, run] * hosp_cost * util * tx_hosp)) * discount[:, 1]
        ## HCC Surveillance Costs (assume 50% HR covered under UHC; 25% resource utilization)
        hcc_cost_bl[:, run] = ((res["pop_hcc"][:, run] * hcc_cost * util) + (
                    res["tgt_hcc"][:, run] * hcc_cost * util) + (
                                           res["tgt_hcc_b"][:, run] * hcc_prp * hcc_cost * util)) * discount[:, 1]
        hcc_cost_s1[:, run] = ((res_s1["pop_hcc"][:, run] * hcc_cost * util) + (
                    res_s1["tgt_hcc"][:, run] * hcc_cost * util) + (
                                           res_s1["tgt_hcc_b"][:, run] * hcc_prp * hcc_cost * util)) * discount[:, 1]
        hcc_cost_s2[:, run] = ((res_s2["pop_hcc"][:, run] * hcc_cost * util) + (
                    res_s2["tgt_hcc"][:, run] * hcc_cost * util) + (
                                           res_s2["tgt_hcc_b"][:, run] * hcc_prp * hcc_cost * util)) * discount[:, 1]
        hcc_cost_s3[:, run] = ((res_s3["pop_hcc"][:, run] * hcc_cost * util) + (
                    res_s3["tgt_hcc"][:, run] * hcc_cost * util) + (
                                           res_s3["tgt_hcc_b"][:, run] * hcc_prp * hcc_cost * util)) * discount[:, 1]
    ...
    for t in range(1, len(cbl_deaths)):
        ...
        ghosts_cbl[t, 1] = max(0,
                               ghosts_cbl[t - 1, 1] - ppl_who_age[1] + ppl_who_age[0] - all_cause_mort[1] * ghosts_cbl[
                                   t - 1, 1])
        ghosts_cbl[t, 2] = max(0,
                               ghosts_cbl[t - 1, 2] - ppl_who_age[2] + ppl_who_age[1] - all_cause_mort[2] * ghosts_cbl[
                                   t - 1, 2])
        ghosts_cbl[t, 3] = max(0,
                               ghosts_cbl[t - 1, 3] - ppl_who_age[3] + ppl_who_age[2] - all_cause_mort[3] * ghosts_cbl[
                                   t - 1, 3])
    ...
    for run in range(runs):
        for t in range(1, len(bl_deaths)):
            ...
            ghosts_bl[run, t, 0] = max(0, ghosts_bl[run, t - 1, 0] - ppl_who_age[0] - all_cause_mort[0] * ghosts_bl[
                run, t - 1, 0])
            ghosts_bl[run, t, 1] = max(0,
                                       ghosts_bl[run, t - 1, 1] - ppl_who_age[1] + ppl_who_age[0] - all_cause_mort[1] *
                                       ghosts_bl[run, t - 1, 1])
            ghosts_bl[run, t, 2] = max(0,
                                       ghosts_bl[run, t - 1, 2] - ppl_who_age[2] + ppl_who_age[1] - all_cause_mort[2] *
                                       ghosts_bl[run, t - 1, 2])
            ghosts_bl[run, t, 3] = max(0,
                                       ghosts_bl[run, t - 1, 3] - ppl_who_age[3] + ppl_who_age[2] - all_cause_mort[3] *
                                       ghosts_bl[run, t - 1, 3])
            ghosts_bl[run, t, 4] = max(0, ghosts_bl[run, t - 1, 4] + ppl_who_age[3] - all_cause_mort[4] * ghosts_bl[
                run, t - 1, 4])
    ...
    for t in range(1, len(cs1_deaths)):
        ...
        ghosts_cs1[t, 1] = max(0,
                               ghosts_cs1[t - 1, 1] - ppl_who_age[1] + ppl_who_age[0] - all_cause_mort[1] * ghosts_cs1[
                                   t - 1, 1])
        ghosts_cs1[t, 2] = max(0,
                               ghosts_cs1[t - 1, 2] - ppl_who_age[2] + ppl_who_age[1] - all_cause_mort[2] * ghosts_cs1[
                                   t - 1, 2])
        ghosts_cs1[t, 3] = max(0,
                               ghosts_cs1[t - 1, 3] - ppl_who_age[3] + ppl_who_age[2] - all_cause_mort[3] * ghosts_cs1[
                                   t - 1, 3])
    ...
    for run in range(runs):
        for t in range(1, len(s1_deaths)):
            ...
            ghosts_s1[run, t, 0] = max(0, ghosts_s1[run, t - 1, 0] - ppl_who_age[0] - all_cause_mort[0] * ghosts_s1[
                run, t - 1, 0])
            ghosts_s1[run, t, 1] = max(0,
                                       ghosts_s1[run, t - 1, 1] - ppl_who_age[1] + ppl_who_age[0] - all_cause_mort[1] *
                                       ghosts_s1[run, t - 1, 1])
            ghosts_s1[run, t, 2] = max(0,
                                       ghosts_s1[run, t - 1, 2] - ppl_who_age[2] + ppl_who_age[1] - all_cause_mort[2] *
                                       ghosts_s1[run, t - 1, 2])
            ghosts_s1[run, t, 3] = max(0,
                                       ghosts_s1[run, t - 1, 3] - ppl_who_age[3] + ppl_who_age[2] - all_cause_mort[3] *
                                       ghosts_s1[run, t - 1, 3])
            ghosts_s1[run, t, 4] = max(0, ghosts_s1[run, t - 1, 4] + ppl_who_age[3] - all_cause_mort[4] * ghosts_s1[
                run, t - 1, 4])
    ...
    for t in range(1, len(cs2_deaths)):
        ...
        ghosts_cs2[t, 1] = max(0,
                               ghosts_cs2[t - 1, 1] - ppl_who_age[1] + ppl_who_age[0] - all_cause_mort[1] * ghosts_cs2[
                                   t - 1, 1])
        ghosts_cs2[t, 2] = max(0,
                               ghosts_cs2[t - 1, 2] - ppl_who_age[2] + ppl_who_age[1] - all_cause_mort[2] * ghosts_cs2[
                                   t - 1, 2])
        ghosts_cs2[t, 3] = max(0,
                               ghosts_cs2[t - 1, 3] - ppl_who_age[3] + ppl_who_age[2] - all_cause_mort[3] * ghosts_cs2[
                                   t - 1, 3])
    ...
    for run in range(runs):
        for t in range(1, len(s2_deaths)):
            ...
            ghosts_s2[run, t, 0] = max(0, ghosts_s2[run, t - 1, 0] - ppl_who_age[0] - all_cause_mort[0] * ghosts_s2[
                run, t - 1, 0])
            ghosts_s2[run, t, 1] = max(0,
                                       ghosts_s2[run, t - 1, 1] - ppl_who_age[1] + ppl_who_age[0] - all_cause_mort[1] *
                                       ghosts_s2[run, t - 1, 1])
            ghosts_s2[run, t, 2] = max(0,
                                       ghosts_s2[run, t - 1, 2] - ppl_who_age[2] + ppl_who_age[1] - all_cause_mort[2] *
                                       ghosts_s2[run, t - 1, 2])
            ghosts_s2[run, t, 3] = max(0,
                                       ghosts_s2[run, t - 1, 3] - ppl_who_age[3] + ppl_who_age[2] - all_cause_mort[3] *
                                       ghosts_s2[run, t - 1, 3])
            ghosts_s2[run, t, 4] = max(0, ghosts_s2[run, t - 1, 4] + ppl_who_age[3] - all_cause_mort[4] * ghosts_s2[
                run, t - 1, 4])
    ...
    for t in range(1, len(cs3_deaths)):
        ...
        ghosts_cs3[t, 1] = max(0,
                               ghosts_cs3[t - 1, 1] - ppl_who_age[1] + ppl_who_age[0] - all_cause_mort[1] * ghosts_cs3[
                                   t - 1, 1])
        ghosts_cs3[t, 2] = max(0,
                               ghosts_cs3[t - 1, 2] - ppl_who_age[2] + ppl_who_age[1] - all_cause_mort[2] * ghosts_cs3[
                                   t - 1, 2])
        ghosts_cs3[t, 3] = max(0,
                               ghosts_cs3[t - 1, 3] - ppl_who_age[3] + ppl_who_age[2] - all_cause_mort[3] * ghosts_cs3[
                                   t - 1, 3])
    ...
    for run in range(runs):
        for t in range(1, len(s3_deaths)):
            ...
            ghosts_s3[run, t, 0] = max(0, ghosts_s3[run, t - 1, 0] - ppl_who_age[0] - all_cause_mort[0] * ghosts_s3[
                run, t - 1, 0])
            ghosts_s3[run, t, 1] = max(0,
                                       ghosts_s3[run, t - 1, 1] - ppl_who_age[1] + ppl_who_age[0] - all_cause_mort[1] *
                                       ghosts_s3[run, t - 1, 1])
            ghosts_s3[run, t, 2] = max(0,
                                       ghosts_s3[run, t - 1, 2] - ppl_who_age[2] + ppl_who_age[1] - all_cause_mort[2] *
                                       ghosts_s3[run, t - 1, 2])
            ghosts_s3[run, t, 3] = max(0,
                                       ghosts_s3[run, t - 1, 3] - ppl_who_age[3] + ppl_who_age[2] - all_cause_mort[3] *
                                       ghosts_s3[run, t - 1, 3])
            ghosts_s3[run, t, 4] = max(0, ghosts_s3[run, t - 1, 4] + ppl_who_age[3] - all_cause_mort[4] * ghosts_s3[
                run, t - 1, 4])
...
def epi_plots(cbl, cs1, cs2, cs3, bl, s1, s2, s3, wd, calib, reg):
    ...
    if reg != "global":
        ...
        bd.fill_between(np.arange(1990, 2099, 1).T, np.percentile(bl["bd_cov"], 2.5, axis=1) * 1e2,
                        np.percentile(bl["bd_cov"], 97.5, axis=1) * 1e2, color=colours[0], alpha=0.2)
        ...
        bd.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s1["bd_cov"], 2.5, axis=1) * 1e2,
                        np.percentile(s1["bd_cov"], 97.5, axis=1) * 1e2, color=colours[1], alpha=0.2)
        ...
        bd.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s2["bd_cov"], 2.5, axis=1) * 1e2,
                        np.percentile(s2["bd_cov"], 97.5, axis=1) * 1e2, color=colours[2], alpha=0.2)
        ...
        bd.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s3["bd_cov"], 2.5, axis=1) * 1e2,
                        np.percentile(s3["bd_cov"], 97.5, axis=1) * 1e2, color=colours[3], alpha=0.2)
        ...
        hb3.fill_between(np.arange(1990, 2099, 1).T, np.percentile(bl["hb3_cov"], 2.5, axis=1) * 1e2,
                         np.percentile(bl["hb3_cov"], 97.5, axis=1) * 1e2, color=colours[0], alpha=0.2)
        ...
        hb3.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s1["hb3_cov"], 2.5, axis=1) * 1e2,
                         np.percentile(s1["hb3_cov"], 97.5, axis=1) * 1e2, color=colours[1], alpha=0.2)
        ...
        hb3.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s2["hb3_cov"], 2.5, axis=1) * 1e2,
                         np.percentile(s2["hb3_cov"], 97.5, axis=1) * 1e2, color=colours[2], alpha=0.2)
        ...
        hb3.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s3["hb3_cov"], 2.5, axis=1) * 1e2,
                         np.percentile(s3["hb3_cov"], 97.5, axis=1) * 1e2, color=colours[3], alpha=0.2)
        ...
        dx.fill_between(np.arange(1990, 2099, 1).T, np.percentile(bl["dx_prop"], 2.5, axis=1) * 1e2,
                        np.percentile(bl["dx_prop"], 97.5, axis=1) * 1e2, color=colours[0], alpha=0.2)
        ...
        dx.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s1["dx_prop"], 2.5, axis=1) * 1e2,
                        np.percentile(s1["dx_prop"], 97.5, axis=1) * 1e2, color=colours[1], alpha=0.2)
        ...
        dx.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s2["dx_prop"], 2.5, axis=1) * 1e2,
                        np.percentile(s2["dx_prop"], 97.5, axis=1) * 1e2, color=colours[2], alpha=0.2)
        ...
        dx.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s3["dx_prop"], 2.5, axis=1) * 1e2,
                        np.percentile(s3["dx_prop"], 97.5, axis=1) * 1e2, color=colours[3], alpha=0.2)
        ...
        tx.fill_between(np.arange(1990, 2099, 1).T, np.percentile(bl["tx_prop"], 2.5, axis=1) * 1e2,
                        np.percentile(bl["tx_prop"], 97.5, axis=1) * 1e2, color=colours[0], alpha=0.2)
        tx.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s1["tx_prop"], 2.5, axis=1) * 1e2,
                        np.percentile(s1["tx_prop"], 97.5, axis=1) * 1e2, color=colours[1], alpha=0.2)
        tx.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s2["tx_prop"], 2.5, axis=1) * 1e2,
                        np.percentile(s2["tx_prop"], 97.5, axis=1) * 1e2, color=colours[2], alpha=0.2)
        tx.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s3["tx_prop"], 2.5, axis=1) * 1e2,
                        np.percentile(s3["tx_prop"], 97.5, axis=1) * 1e2, color=colours[3], alpha=0.2)
        ...
        tx.legend(("Status Quo", "Coverage Targets Achieved in 2030", "Coverage Targets Achieved in 2040",
                   "Coverage Targets Achieved in 2050"), fontsize="large", frameon=False, loc="upper left")
    ...
    prev.fill_between(np.arange(1990, 2099, 1).T, np.percentile(bl["prev"], 2.5, axis=1) * 1e2,
                      np.percentile(bl["prev"], 97.5, axis=1) * 1e2, color=colours[0], alpha=0.2)
    ...
    prev.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s1["prev"], 2.5, axis=1) * 1e2,
                      np.percentile(s1["prev"], 97.5, axis=1) * 1e2, color=colours[1], alpha=0.2)
    ...
    prev.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s2["prev"], 2.5, axis=1) * 1e2,
                      np.percentile(s2["prev"], 97.5, axis=1) * 1e2, color=colours[2], alpha=0.2)
    ...
    prev.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s3["prev"], 2.5, axis=1) * 1e2,
                      np.percentile(s3["prev"], 97.5, axis=1) * 1e2, color=colours[3], alpha=0.2)
    ...
    prev_u5.fill_between(np.arange(1990, 2099, 1).T, np.percentile(bl["prev_u5"], 2.5, axis=1) * 1e2,
                         np.percentile(bl["prev_u5"], 97.5, axis=1) * 1e2, color=colours[0], alpha=0.2)
    ...
    prev_u5.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s1["prev_u5"], 2.5, axis=1) * 1e2,
                         np.percentile(s1["prev_u5"], 97.5, axis=1) * 1e2, color=colours[1], alpha=0.2)
    ...
    prev_u5.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s2["prev_u5"], 2.5, axis=1) * 1e2,
                         np.percentile(s2["prev_u5"], 97.5, axis=1) * 1e2, color=colours[2], alpha=0.2)
    ...
    prev_u5.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s3["prev_u5"], 2.5, axis=1) * 1e2,
                         np.percentile(s3["prev_u5"], 97.5, axis=1) * 1e2, color=colours[3], alpha=0.2)
    ...
    mort.fill_between(np.arange(1990, 2099, 1).T, np.percentile(bl["mort"], 2.5, axis=1) / 1e3,
                      np.percentile(bl["mort"], 97.5, axis=1) / 1e3, color=colours[0], alpha=0.2)
    ...
    mort.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s1["mort"], 2.5, axis=1) / 1e3,
                      np.percentile(s1["mort"], 97.5, axis=1) / 1e3, color=colours[1], alpha=0.2)
    ...
    mort.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s2["mort"], 2.5, axis=1) / 1e3,
                      np.percentile(s2["mort"], 97.5, axis=1) / 1e3, color=colours[2], alpha=0.2)
    ...
    mort.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s3["mort"], 2.5, axis=1) / 1e3,
                      np.percentile(s3["mort"], 97.5, axis=1) / 1e3, color=colours[3], alpha=0.2)
    ...
    hcc.fill_between(np.arange(1990, 2099, 1).T, np.percentile(bl["hcc_inc"], 2.5, axis=1) / 1e3,
                     np.percentile(bl["hcc_inc"], 97.5, axis=1) / 1e3, color=colours[0], alpha=0.2)
    hcc.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s1["hcc_inc"], 2.5, axis=1) / 1e3,
                     np.percentile(s1["hcc_inc"], 97.5, axis=1) / 1e3, color=colours[1], alpha=0.2)
    hcc.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s2["hcc_inc"], 2.5, axis=1) / 1e3,
                     np.percentile(s2["hcc_inc"], 97.5, axis=1) / 1e3, color=colours[2], alpha=0.2)
    hcc.fill_between(np.arange(1990, 2099, 1).T, np.percentile(s3["hcc_inc"], 2.5, axis=1) / 1e3,
                     np.percentile(s3["hcc_inc"], 97.5, axis=1) / 1e3, color=colours[3], alpha=0.2)
    ...
    plt.gca().yaxis.set_major_formatter(plt.matplotlib.ticker.StrMethodFormatter('{x:,.0f}'))
    hcc.legend(("Status Quo", "Coverage Targets Achieved in 2030", "Coverage Targets Achieved in 2040",
                "Coverage Targets Achieved in 2050"), fontsize="large", frameon=False)
    # plt.grid()
...
def econ_plots(econ_dict, reg):
    ...
    daly.fill_between(np.arange(1990, 2099, 1).T, np.percentile(econ_dict["bl_daly"], 2.5, axis=1) / 1e6,
                      np.percentile(econ_dict["bl_daly"], 97.5, axis=1) / 1e6, color=colours[0], alpha=0.2)
    daly.fill_between(np.arange(1990, 2099, 1).T, np.percentile(econ_dict["s1_daly"], 2.5, axis=1) / 1e6,
                      np.percentile(econ_dict["s1_daly"], 97.5, axis=1) / 1e6, color=colours[1], alpha=0.2)
    daly.fill_between(np.arange(1990, 2099, 1).T, np.percentile(econ_dict["s2_daly"], 2.5, axis=1) / 1e6,
                      np.percentile(econ_dict["s2_daly"], 97.5, axis=1) / 1e6, color=colours[2], alpha=0.2)
    daly.fill_between(np.arange(1990, 2099, 1).T, np.percentile(econ_dict["s3_daly"], 2.5, axis=1) / 1e6,
                      np.percentile(econ_dict["s3_daly"], 97.5, axis=1) / 1e6, color=colours[3], alpha=0.2)
    ...
    daly.legend(("Status Quo", "Coverage Targets Achieved in 2030", "Coverage Targets Achieved in 2040",
                 "Coverage Targets Achieved in 2050"), fontsize="large", frameon=False)
    ...
    dc.fill_between(np.arange(1990, 2099, 1).T, np.percentile(econ_dict["bl_dirc"], 2.5, axis=1) / 1e9,
                    np.percentile(econ_dict["bl_dirc"], 97.5, axis=1) / 1e9, color=colours[0], alpha=0.2)
    ...
    dc.fill_between(np.arange(1990, 2099, 1).T, np.percentile(econ_dict["s1_dirc"], 2.5, axis=1) / 1e9,
                    np.percentile(econ_dict["s1_dirc"], 97.5, axis=1) / 1e9, color=colours[1], alpha=0.2)
    ...
    dc.fill_between(np.arange(1990, 2099, 1).T, np.percentile(econ_dict["s2_dirc"], 2.5, axis=1) / 1e9,
                    np.percentile(econ_dict["s2_dirc"], 97.5, axis=1) / 1e9, color=colours[2], alpha=0.2)
    ...
    dc.fill_between(np.arange(1990, 2099, 1).T, np.percentile(econ_dict["s3_dirc"], 2.5, axis=1) / 1e9,
                    np.percentile(econ_dict["s3_dirc"], 97.5, axis=1) / 1e9, color=colours[3], alpha=0.2)
    ...
    icer.fill_between(np.arange(2025, 2051, 1).T, np.percentile(econ_dict["s1_icer"][35:61, :], 2.5, axis=1) / 1e3,
                      np.percentile(econ_dict["s1_icer"][35:61, :], 97.5, axis=1) / 1e3, color=colours[1], alpha=0.2)
    ...
    icer.fill_between(np.arange(2025, 2051, 1).T, np.percentile(econ_dict["s2_icer"][35:61, :], 2.5, axis=1) / 1e3,
                      np.percentile(econ_dict["s2_icer"][35:61, :], 97.5, axis=1) / 1e3, color=colours[2], alpha=0.2)
    ...
    icer.fill_between(np.arange(2025, 2051, 1).T, np.percentile(econ_dict["s3_icer"][35:61, :], 2.5, axis=1) / 1e3,
                      np.percentile(econ_dict["s3_icer"][35:61, :], 97.5, axis=1) / 1e3, color=colours[3], alpha=0.2)
    ...
    neb.fill_between(np.arange(2022, 2051, 1).T, np.percentile(econ_dict["s1_neb"][32:61, :], 2.5, axis=1) / 1e9,
                     np.percentile(econ_dict["s1_neb"][32:61, :], 97.5, axis=1) / 1e9, color=colours[1], alpha=0.2)
    ...
    neb.fill_between(np.arange(2022, 2051, 1).T, np.percentile(econ_dict["s2_neb"][
32:61, :], 2.5, axis=1) / 1e9,
                     np.percentile(econ_dict["s2_neb"][32:61, :], 97.5, axis=1) / 1e9, color=colours[2], alpha=0.2)
    ...
    neb.fill_between(np.arange(2022, 2051, 1).T, np.percentile(econ_dict["s3_neb"][32:61, :], 2.5, axis=1) / 1e9,
                     np.percentile(econ_dict["s3_neb"][32:61, :], 97.5, axis=1) / 1e9, color=colours[3], alpha=0.2)
...
def scenario_edit(D, par, cov_dict=defaultdict(lambda: []), t_dict=defaultdict(lambda: []),
...
def pop_calib(P, cal, maxtime=300):
    ...
    '''
    return P.calibrate(max_time=maxtime, parset=cal, adjustables=["acm"], measurables=["alive"], default_min_scale=0.5,
                       default_max_scale=2)
...
def epi_calib(P, cal, maxtime=100, ranges=4):
    ...
    for i in range(ranges):  # lets iterate a few more times
        ...
        cal = P.calibrate(max_time=maxtime, parset=cal, adjustables=["sag_ix", "ad_pop_sus", "ch_pop_sus", "eag_ix"],
                          measurables=["prev"], default_min_scale=0.1, default_max_scale=5)
        cal = P.calibrate(max_time=maxtime, parset=cal, adjustables=["m_acu"], measurables=["cl_acu"],
                          default_min_scale=0.1, default_max_scale=5)
        cal = P.calibrate(max_time=maxtime, parset=cal, adjustables=["it_icl", "icl_ict"], measurables=["eag_ott"],
                          default_min_scale=0.1, default_max_scale=3)

        print(f'RUN NUMBER {i + 1}, cirr and hcc')
        # Testing this new parameter (lower and upper bounds can be adjusted)
        cal = P.calibrate(max_time=maxtime, parset=cal, adjustables=["cc_dc", "dc_hcc"],
                          measurables=["cl_cir", 'cl_hcc'], default_min_scale=0.1, default_max_scale=3)
        cal = P.calibrate(max_time=maxtime, parset=cal, adjustables=["m_dc"], measurables=["cl_cir"],
                          default_min_scale=0.1, default_max_scale=5)
        cal = P.calibrate(max_time=maxtime, parset=cal, adjustables=["m_hcc"], measurables=["cl_hcc"],
                          default_min_scale=0.1, default_max_scale=5)
    return cal
...
def determine_cal_quality(df):
    ...
    df = df.style.apply(highlight_results, lb=1, ub=1000, props="color:white;background-color:black", axis=0).apply(
        highlight_results, lb=0.6, ub=1, props="color:white;background-color:red", axis=0).apply(highlight_results,
                                                                                                 lb=0.4, ub=0.6,
                                                                                                 props="color:black;background-color:gold",
                                                                                                 axis=0).apply(
        highlight_results, lb=0, ub=0.4, props="color:black;background-color:mediumseagreen", axis=0)
    return df
...
def error_table(regions, app='vimc', fw_loc='frameworks/hbv_v14_gamma_mav.xlsx'):
    '''
    ...
    for ct in regions:
        ...
        if app == 'region':
            ...
            D = at.ProjectData.from_spreadsheet(f"applications/{app}_{ct.lower()}/{ct}_db.xlsx", framework=F)
        ...
        if app == 'region':
            ...
            cal.load_calibration(f"applications/{app}_{ct.lower()}/{ct}_calib.xlsx")
